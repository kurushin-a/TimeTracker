...
const STATUS_LABELS={open:"Открыта",in_progress:"В процессе",done:"Выполнена",cancelled:"Отменена",active:"Активная",paused:"На паузе"};
const STATUS_CLASS={open:"s-open",in_progress:"s-progress",done:"s-done",cancelled:"s-cancelled",active:"s-open",paused:"s-paused"};
...
async function loadLogPage(iso) {
  ...
  await Promise.all([loadDayIntoForm(iso), loadDayTasks(iso), loadSuggestions(iso)]);
}
...
function addSleepRow(data = {}) {
  ...
  const sleepDateIn = row.querySelector(".sleep-date");
  const sleepTimeIn = row.querySelector(".sleep-time");
  const wakeDateIn  = row.querySelector(".wake-date");
  const wakeTimeIn  = row.querySelector(".wake-time");
  const badge       = row.querySelector(".sleep-duration-badge");

  // дефолты: лёг вчера, встал сегодня
...
function updateSleepSummary() {
  ...
  // сон "до дня" — сессии где проснулся в этот день
  const before = sessions.filter(s => s.wake_date === iso);
  // общий — все уникальные сессии связанные с днём
  const all = [...new Map(
    sessions
      .filter(s => s.wake_date === iso || s.sleep_date === iso)
      .map(s => [`${s.sleep_date}${s.sleep_time}`, s])
  ).values()];

  const calcMin = s => {
    const start = new Date(`${s.sleep_date}T${s.sleep_time}`);
    const end   = new Date(`${s.wake_date}T${s.wake_time}`);
...
function addActivityRow(data={}) {
  ...
  openTasks.forEach(t=>{const o=document.createElement("option");o.value=t.id;o.textContent=t.title.length>28?t.title.slice(0,28)+"…":t.title;sel.appendChild(o);});
...
function collectActivities() {
  return [...actList.querySelectorAll(".activity-row")].map(row=>({
    title:row.querySelector(".act-title").value.trim(), category:row.querySelector(".act-category").value,
    impact:row.querySelector(".act-impact").value, duration_m:parseInt(row.querySelector(".act-duration").value)||0,
    task_id:row.querySelector(".act-task-id").value||null, note:row.querySelector(".act-note").value.trim()||null,
...
async function loadDayTasks(iso) {
  ...
  if(!active.length){el.innerHTML="<em class='muted'>Нет активных задач</em>";return;}
  el.innerHTML="";
  active.forEach(t=>{
    const row=document.createElement("div"); row.className="task-row"+(t.status==="done"?" task-done":"");
    row.innerHTML=`<input type="checkbox" class="task-cb" ${t.status==="done"?"checked":""}/>
      <span class="task-title">${t.title}</span><span class="detail-tag cat-${t.category}">${CAT_LABELS[t.category]||t.category}</span>
    ...
    row.querySelector(".task-cb").addEventListener("change",async e=>{
      await fetch(`${API}/tasks/${t.id}/status?status=${e.target.checked?"done":"open"}`,{method:"PATCH"});
      row.classList.toggle("task-done",e.target.checked); await refreshOpenTasks();
    });
    el.appendChild(row);
  });
}

async function refreshOpenTasks() { openTasks=await fetch(`${API}/tasks/open`).then(r=>r.json()).catch(()=>[]); }
...
document.getElementById("save-day-btn").addEventListener("click",async()=>{
  ...
  const payload={
    date:dateInput.value, activities:collectActivities(),
    habits_done, rating:parseInt(ratingInput.value),
    note:document.getElementById("note").value.trim()||null,
    sleep_sessions:collectSleepSessions(),
  };
...
async function refreshGoals() { allGoals=await fetch(`${API}/goals`).then(r=>r.json()).catch(()=>[]); populateGoalSelects(); }
...
async function openGoalDetail(goalId){
  ...
  const [prog,tree]=await Promise.all([fetch(`${API}/goals/${goalId}/progress`).then(r=>r.json()),fetch(`${API}/tasks/tree?goal_id=${goalId}`).then(r=>r.json()).catch(()=>[])]);
  const g=prog.goal;
  function renderNode(node,depth=0){const t=node.task,est=t.estimated_minutes?`~${fmtMinutes(t.estimated_minutes)}`:"",act=t.actual_minutes?`${fmtMinutes(t.actual_minutes)} факт`:"";return`<div class="tree-node" style="padding-left:${depth*20}px"><span class="status-dot ${STATUS_CLASS[t.status]||""}"></span><span class="tree-title ${t.status==="done"?"line-through":""}">${t.title}</span><span class="detail-tag cat-${t.category}">${CAT_LABELS[t.category]||t.category}</span>${est?`<span class="task-time-badge">${est}</span>`:""}${act?`<span class="task-time-badge act">${act}</span>`:""}<button class="btn-link task-status-toggle" data-id="${t.id}" data-done="${t.status==="done"}">${t.status==="done"?"↩ Открыть":"✓ Готово"}</button></div>${node.children.map(c=>renderNode(c,depth+1)).join("")}`;}
  content.innerHTML=`<div class="card" style="margin-top:.5rem"><div class="goal-card-header"><span class="goal-title">${g.title}</span><span class="status-badge ${STATUS_CLASS[g.status]}">${STATUS_LABELS[g.status]||g.status}</span></div>${g.description?`<p class="goal-desc">${g.description}</p>`:""}<div class="goal-meta">${g.deadline?`<span>📅 ${localDate(g.deadline).toLocaleDateString("ru-RU",{day:"numeric",month:"long",year:"numeric"})}</span>`:""}<span>⏱ ${fmtMinutes(prog.total_minutes)} вложено</span><span>✅ ${prog.tasks_done}/${prog.tasks_total} задач</span></div></div><div class="card"><h3>Задачи</h3><div class="task-tree">${tree.length?tree.map(n=>renderNode(n)).join(""):"<em class='muted'>Нет задач</em>"}</div><button class="btn-secondary" style="margin-top:.5rem" onclick="switchPage('tasks')">+ Добавить задачу</button></div><div class="card"><h3>Привычки</h3>${prog.linked_habits.length?prog.linked_habits.map(h=>`<div class="habit-row"><span class="habit-title">${h.title}</span><span class="habit-freq">${h.frequency==="daily"?"каждый день":`${h.times_per_week}× в нед.`}</span></div>`).join(""):"<em class='muted'>Нет привычек</em>"}</div>`;
  content.querySelectorAll(".task-status-toggle").forEach(btn=>btn.addEventListener("click",async()=>{const isDone=btn.dataset.done==="true";await fetch(`${API}/tasks/${btn.dataset.id}/status?status=${isDone?"open":"done"}`,{method:"PATCH"});openGoalDetail(goalId);}));
}

// ─── TASKS PAGE ───────────────────────────────────────────────────────────────

let tasksInited=false,taskFilterStatus="open,in_progress";
...
async function initTasksPage(){
  ...
  if(!tasksInited){
    ...
    document.querySelectorAll("[data-task-status]").forEach(btn=>btn.addEventListener("click",()=>{document.querySelectorAll("[data-task-status]").forEach(b=>b.classList.remove("active"));btn.classList.add("active");taskFilterStatus=btn.dataset.taskStatus;renderTasks();}));
    tasksInited=true;
  }
  populateGoalSelects();renderTasks();
}
...
async function renderTasks(){
  ...
  if(taskFilterStatus){const a=taskFilterStatus.split(",");tasks=tasks.filter(t=>a.includes(t.status));}
  if(!tasks.length){el.innerHTML="<div class='card'><em class='muted'>Нет задач</em></div>";return;}
  el.innerHTML="";
  tasks.forEach(t=>{
    const goalName=allGoals.find(g=>g.id===t.goal_id)?.title;
    const est=t.estimated_minutes?fmtMinutes(t.estimated_minutes):null,act=t.actual_minutes?fmtMinutes(t.actual_minutes):null;
    const eff=(t.estimated_minutes&&t.actual_minutes)?Math.round(t.actual_minutes/t.estimated_minutes*100):null;
    const card=document.createElement("div");card.className="card task-card";
    card.innerHTML=`<div class="task-card-header"><div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap"><span class="status-dot ${STATUS_CLASS[t.status]||""}"></span><span class="task-card-title">${t.title}</span><span class="detail-tag cat-${t.category}">${CAT_LABELS[t.category]||t.category}</span>${goalName?`<span class="goal-ref">🎯 ${goalName}</span>`:""}</div><button class="btn-link danger task-del" data-id="${t.id}">✕</button></div>${t.note?`<div class="task-note-text">${t.note}</div>`:""}<div class="task-time-row">${est?`<span class="task-time-badge">план: ${est}</span>`:""} ${act?`<span class="task-time-badge act">факт: ${act}</span>`:""} ${eff!==null?`<span class="task-time-badge ${eff>100?"over":""}">эфф: ${eff}%</span>`:""} ${t.deadline?`<span class="task-time-badge">📅 ${localDate(t.deadline).toLocaleDateString("ru-RU",{day:"numeric",month:"short"})}</span>`:""}</div><div class="task-status-row">${["open","in_progress","done"].map(s=>`<button class="status-btn ${t.status===s?"active":""}" data-id="${t.id}" data-s="${s}">${STATUS_LABELS[s]}</button>`).join("")}</div>`;
    card.querySelectorAll(".status-btn").forEach(btn=>btn.addEventListener("click",async()=>{await fetch(`${API}/tasks/${btn.dataset.id}/status?status=${btn.dataset.s}`,{method:"PATCH"});renderTasks();await refreshOpenTasks();}));
    card.querySelector(".task-del").addEventListener("click",async()=>{if(!confirm(`Удалить «${t.title}»?`))return;await fetch(`${API}/tasks/${t.id}`,{method:"DELETE"});renderTasks();});
    el.appendChild(card);
  });
}

async function saveTask(){
  const payload={title:document.getElementById("task-title").value.trim(),category:document.getElementById("task-category").value,deadline:document.getElementById("task-deadline").value||null,estimated_minutes:parseInt(document.getElementById("task-estimated").value)||null,goal_id:document.getElementById("task-goal-id").value||null,note:document.getElementById("task-note").value.trim()||null,status:"open"};
  if(!payload.title)return;
  await fetch(`${API}/tasks`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
  ["task-title","task-estimated","task-note"].forEach(id=>document.getElementById(id).value="");
  document.getElementById("add-task-form").style.display="none";
  renderTasks();await refreshOpenTasks();
}
...
function renderCalendar(){
  ...
  for(let day=1;day<=totalDays;day++){
    ...
    // показываем рейтинг и иконку сна если есть
...
async function showDayDetail(iso){
  ...
  const sleepHtml = day.sleep_before_day ? `
    <div class="detail-sleep">
      💤 Сон до дня: <b>${fmtMinutes(day.sleep_before_day)}</b>
      ${day.sleep_total!==day.sleep_before_day?` · Общий: <b>${fmtMinutes(day.sleep_total)}</b>`:""}
      ${day.wake_time?` · Подъём: <b>${day.wake_time}</b>`:""}
      ${day.untracked_minutes?` · В хламе: <b>${fmtMinutes(day.untracked_minutes)}</b>`:""}
    </div>` : "";

  detail.innerHTML=`
    <div class="detail-header"><strong>${dateStr}</strong><span class="detail-rating-badge" style="background:${ratingColor(day.rating)};color:${ratingTextColor(day.rating)}">${day.rating}/10</span></div>
    ${sleepHtml}
    ${day.note?`<p class="detail-day-note">${day.note}</p>`:""}
    <div class="detail-acts">${day.activities.map(a=>`<div class="detail-act"><span class="detail-act-title">${a.title}</span><span class="detail-tag cat-${a.category}">${CAT_LABELS[a.category]||a.category}</span><span class="detail-tag imp-${a.impact}">${IMPACT_LABELS[a.impact]||a.impact}</span><span class="detail-dur">${a.duration_m}м</span>${a.note?`<span class="detail-note">${a.note}</span>`:""}</div>`).join("")||"<em>Нет действий</em>"}</div>
    <div style="text-align:right;margin-top:.5rem"><button class="btn-secondary" onclick="openLogForDate('${iso}')">✏️ Редактировать</button></div>`;
...
function renderStats(el,s){
  ...
  el.innerHTML=`
    <div class="stat-grid">
      ...
      ${s.avg_sleep_minutes?`<div class="stat-box"><div class="val">${fmtMinutes(Math.round(s.avg_sleep_minutes))}</div><div class="lbl">Средний сон</div></div>`:""}
      ${s.avg_wake_time?`<div class="stat-box"><div class="val">${s.avg_wake_time}</div><div class="lbl">Средний подъём</div></div>`:""}
    </div>
    <h3 style="margin-top:1rem">Время по категориям</h3>
    ${Object.entries(s.time_by_category).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div class="bar-row"><span class="bar-label">${CAT_LABELS[k]||k}</span><div class="bar-track"><div class="bar-fill" style="width:${(v/mC*100).toFixed(1)}%"></div></div><span class="bar-val">${fmtMinutes(v)}</span></div>`).join("")}
    <h3 style="margin-top:1rem">Время по импакту</h3>
    ${Object.entries(s.time_by_impact).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div class="bar-row"><span class="bar-label">${IMPACT_LABELS[k]||k}</span><div class="bar-track"><div class="bar-fill" style="width:${(v/mI*100).toFixed(1)}%;background:${IMPACT_COLORS[k]||"#999"}"></div></div><span class="bar-val">${fmtMinutes(v)}</span></div>`).join("")}`;
}
